...
public:
    ...
    std::unordered_set<ChessSquare*> getImmediateSurrounding(int x, int y); 
    ...
    void addConnection(int fromIndex, std::unordered_set<int> nextIndexes);
    std::vector<ChessSquare*> getConnectedSquares(int x, int y);
    bool areConnected(int fromX, int fromY, int toX, int toY);
    bool areConnected(int fromIndex, int toIndex);
...
private:
    ...
    std::unordered_map<int, std::unordered_set<int>> _connections;
